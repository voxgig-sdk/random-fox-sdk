...
func TestFoxEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set RANDOM_FOX_TEST_FOX_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		foxRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.fox")))
...
func foxBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RANDOM_FOX_TEST_FOX_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RANDOM_FOX_TEST_FOX_ENTID"])
