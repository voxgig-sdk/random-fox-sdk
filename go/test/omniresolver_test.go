...
import (
	...
	omni "github.com/voxgig-sdk/random-fox-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RandomFoxError); is {
				out := map[string]any{
					"name":    "RandomFoxError",
...
func omniCtx(arg any, client *sdk.RandomFoxSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RandomFoxSDK); is {
			client = live
