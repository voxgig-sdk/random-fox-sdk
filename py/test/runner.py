...
class RandomFoxTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RandomFoxTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RandomFoxTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RandomFoxTestRunner.getenv("RANDOM_FOX_TEST_LIVE")
        override = RandomFoxTestRunner.getenv("RANDOM_FOX_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RandomFoxTestRunner.getenv(key)
        ...
        explain = RandomFoxTestRunner.getenv("RANDOM_FOX_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["RANDOM_FOX_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RandomFoxTestRunner._test_control is not None:
            return RandomFoxTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RandomFoxTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RandomFoxTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RandomFoxTestRunner.load_test_control()
...
def load_env_local():
    RandomFoxTestRunner.load_env_local()


def env_override(m):
    return RandomFoxTestRunner.env_override(m)


def entity_data(v):
    return RandomFoxTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RandomFoxTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RandomFoxTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RandomFoxTestRunner.load_test_control()


def live_client_options():
    return RandomFoxTestRunner.live_client_options()


def live_delay_ms():
    return RandomFoxTestRunner.live_delay_ms()
