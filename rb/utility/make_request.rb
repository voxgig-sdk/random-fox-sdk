...
module RandomFoxUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RandomFoxResponse.new({})
    ...
    if fetch_err
      ...
      response = RandomFoxResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RandomFoxResponse.new(fetched)
    else
