...
class FoxEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = fox_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "fox." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RANDOM_FOX_TEST_FOX_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    fox_ref01_data = nil
    if fox_ref01_data_raw.length > 0
...
def fox_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "fox", "FoxTestData.json")
  ...
  entid_env_raw = ENV["RANDOM_FOX_TEST_FOX_ENTID"]
  ...
  if env["RANDOM_FOX_TEST_LIVE"] == "TRUE"
    ...
    client = RandomFoxSDK.new(Helpers.to_map(merged_opts))
  end
