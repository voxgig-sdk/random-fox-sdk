...
require_relative 'RandomFox_types'
...
class RandomFoxSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RandomFoxHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RandomFoxHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RandomFoxFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RandomFoxUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RandomFoxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RandomFoxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RandomFoxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RandomFoxHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RandomFoxError.new(
        "graphql_error", "RandomFoxSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
