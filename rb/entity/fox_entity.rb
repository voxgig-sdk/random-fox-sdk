...
class FoxEntity
  ...
  def make
    opts = @_entopts.dup
    FoxEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RandomFoxHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Fox, Hash] the current Fox data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RandomFoxHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Fox fields)
  def match_get
  ...
  # Load a single Fox.
  #
  # @param reqmatch [FoxLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Fox.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Fox, Hash] the loaded Fox; raises RandomFoxError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RandomFoxHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
