...
boru-driven command-line client **and** interactive REPL for the RandomFox
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/random-fox-cli)
make build
...
./random-fox-cli load 1 fox            # {id:1} shorthand
./random-fox-cli load '{id:1}' fox       # explicit match map

# 5. Override the API base URL for a single call
RANDOM_FOX_BASE=https://api.example.com ./random-fox-cli load 1 fox

# 6. No arguments -> interactive REPL
./random-fox-cli
random-fox> load 1 fox
random-fox> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/random-fox-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/random-fox-cli load 1 fox
   ```
...
```sh
./random-fox-cli load 1 fox          # scalar shorthand for {id:1}
./random-fox-cli load '{id:1}' fox     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `random-fox>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `fox`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/random-fox-cli`. |
...
(`./random-fox-cli <expr>`) and interactively (the REPL), and expressions compose the
