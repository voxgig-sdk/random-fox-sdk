...
local FoxEntity = {}
FoxEntity.__index = FoxEntity


function FoxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FoxEntity)
  self._name = "fox"
  self._client = client
...
function FoxEntity:get_name()
  return self._name
end


function FoxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FoxEntity.new(self._client, opts)
end
...
function FoxEntity:mark_deleted()
  self._deleted = true
end


function FoxEntity:deleted()
  return true == self._deleted
end


function FoxEntity:data_set(args)
  if args ~= nil then
...
function FoxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FoxEntity:match_set(args)
  if args ~= nil then
...
function FoxEntity:stream(action, args, callopts)
...
function FoxEntity:_run_op(ctx, post_done)
  local utility = self._utility
