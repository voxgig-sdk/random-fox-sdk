...
[MCP](https://modelcontextprotocol.io) server exposing the RandomFox SDK as
two agent tools — `random-fox_list` and `random-fox_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/random-fox-mcp)
make build

# 2. Provide credentials via the environment
export RANDOM_FOX_APIKEY=sk_live_xxx
...
```jsonc
// random-fox_list: first page of records
{ "entity": "fox" }
{ "entity": "fox", "query": { } }

// random-fox_load: one record by id
{ "entity": "fox", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/random-fox-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user random-fox \
     -- "$PWD"/dist/*/random-fox-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `random-fox_list` and `random-fox_load` tools now appear
   in new sessions. Ask the agent to *"list fox using random-fox"*
   and it calls `random-fox_list` with `{"entity":"fox"}`.
...
```sh
export RANDOM_FOX_APIKEY=sk_live_xxx            # API key
export RANDOM_FOX_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `random-fox_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `random-fox_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RANDOM_FOX_APIKEY` | API key sent with every request. |
| `RANDOM_FOX_BASE` | Optional override of the API base URL. |
