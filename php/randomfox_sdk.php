...
class RandomFoxSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = RandomFoxHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = RandomFoxHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!RandomFoxFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, RandomFoxFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return RandomFoxUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = RandomFoxHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = RandomFoxHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = RandomFoxHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new RandomFoxError($op . "_allow",
                "RandomFoxSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = RandomFoxHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new RandomFoxError("graphql_error",
                "RandomFoxSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Fox($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_fox === null) {
                $this->_fox = new FoxEntity($this, null);
            }
            return $this->_fox;
        }
        return new FoxEntity($this, $data);
    }
