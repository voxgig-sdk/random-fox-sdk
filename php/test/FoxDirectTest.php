...
class FoxDirectTest extends TestCase
{
    public function test_direct_load_fox(): void
    {
        $setup = fox_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-fox", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function fox_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new RandomFoxSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
