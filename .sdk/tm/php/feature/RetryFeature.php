...
class RandomFoxRetryFeature extends RandomFoxBaseFeature
{
    ...
    public function init(RandomFoxContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RandomFoxContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RandomFoxContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RandomFoxContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
