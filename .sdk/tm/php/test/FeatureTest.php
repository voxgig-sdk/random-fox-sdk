...
class FtCtrl extends RandomFoxControl
{
...
class FtClient
{
    ...
    private RandomFoxUtility $utility;

    public function __construct(RandomFoxUtility $utility, array $options)
    {
    ...
    public function get_utility(): RandomFoxUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RandomFoxContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RandomFoxContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RandomFoxContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RandomFoxConfig::make_feature($name);
    ...
    private static function buildUrl(RandomFoxSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RandomFoxError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RandomFoxResponse($fetched);
        }
    ...
    private function fail(RandomFoxContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RandomFoxContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
