...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.RANDOMFOX_TEST_LIVE ||
    'TRUE' === process.env.RANDOMFOX_TEST_OVERRIDE
  ) {
  ...
  m.RANDOMFOX_TEST_EXPLAIN = process.env.RANDOMFOX_TEST_EXPLAIN || m.RANDOMFOX_TEST_EXPLAIN
