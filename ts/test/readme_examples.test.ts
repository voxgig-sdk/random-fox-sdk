...
//     (\`new RandomFoxSDK(...)\` / \`RandomFoxSDK.test(...)\` become
//     \`RandomFoxSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"fox":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RandomFoxSDK(...)\` and any
// \`RandomFoxSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RandomFoxSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RandomFoxSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
